...
uses
  System.Rtti, SysUtils, FireDAC.Comp.Client, SolvoRTTIHelper, SolvoAttributes;

class procedure TSolvoUtil.DataSetToObjectList<T>(const poDataSet: TDataSet; const
...
class procedure TSolvoUtil.GetValuesFromDataset(const poDataset: TDataSet;
...
class procedure TSolvoUtil.GetObjectFromForm(const aForm: TForm;
...
class function TSolvoUtil.GetTextFromComponent(aComponent: TComponent): string;
begin
...
class procedure TSolvoUtil.SetFormFromObject(const aForm: TForm;
  const aObject: TObject);
var
  ctxRttiEntity: TRttiContext;
  typRttiEntity: TRttiType;
