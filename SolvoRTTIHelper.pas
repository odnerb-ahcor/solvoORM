unit SolvoRTTIHelper;

interface

uses
  RTTI, SolvoAttributes;
...
function TRttiPropertyHelper.Mask: string;
begin
  Result := '';

  if Tem<FieldMask> then
   Result := GetAttribute<FieldMask>.Mask;
end;
...
function TRttiPropertyHelper.FieldFK: string;
begin
  Result := Name;
  if EhChaveEstrangeira then
    Result := GetAttribute<FK>.Field;
end;
