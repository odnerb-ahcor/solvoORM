...
uses
  System.Classes, RTTI, SolvoAttributes, System.SysUtils, SolvoRTTIHelper;

type
  ESolvoValidator = class(Exception);

  TSolvoValidator = class
  private
...
class procedure TSolvoValidator.Validate(const aObject: TObject; const
  aErrors: TStrings);
var
...
class function TSolvoValidator.IsClassRtti(const aObject: TObject): Boolean;
var
  ctxRttiEntity: TRttiContext;
  typRttiEntity: TRttiType;
begin
...
class procedure TSolvoValidator.Validate(const aObject: TObject);
var
  sErrors: TStringList;
begin
  sErrors := TStringList.Create;
  try
    TSolvoValidator.Validate(aObject, sErrors);
    if sErrors.Count > 0 then
      raise ESolvoValidator.Create('Encontrado erros de preenchimento!'#13 +
...
class procedure TSolvoValidator.ValidateNotNull(const aErrros: TStrings; const
